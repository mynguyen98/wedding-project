...
const CreatePage = () => {
    ...
    const renderRadio = useCallback((id, label, value) => {
    ...
    const renderImageUploadSingle = useCallback((title, images, desc, allowDrag, onChange) => {
    ...
    const renderTitle = (title, divided, classNameCus) => {
        return <div className={`${'title_comp_wed'} ${classNameCus ? classNameCus : ''}`}>
            <h2>
                {title}
    ...
    const renderInput = useCallback((ref, value, label, placehodel, type, maxLength, isIcon, icon, inputStyle) => {

        const onChange = (text, placehodel) => {
            onChangeText(text, placehodel);
        };

        return <div className='item_field_single'>
            <MyTextInput
    ...
    const renderFamilyMan = useMemo(() => {
        ...
        return <div className='total_family_one_side'>
            {renderTitle(Languages.text.man, true)}
            <div className='input_fields_control'>
                <div className='place_title_input'>
                    <label>
                        {Languages.inputText.groom}
                    </label>
                </div>
                <div className='group_input_control'>

                    {renderInput('', '', Languages.inputText.firstNameGroom, Languages.inputText.firstName, 'text', 15, false, false)}
                    {renderInput('', '', Languages.inputText.firstAnother, Languages.inputText.firstAnother, 'text', 30, false, false)}
                    {renderInput('', '', Languages.inputText.nameGroom, Languages.inputText.namesingle, 'text', 30, false, false)}

                    <div className='item_field_single'>
                        <div className='sellect_option'>
                            <label className="Input_label__90o4b">{Languages.inputText.stt}</label>
                            <select className='form_sellect_control' name='form_sellect_stt' value={selectSTT} onChange={onChangeSelectStt}>
                    ...
                    <div className='item_field_single select_code'>
                        <div className='sellect_option select_phone_code'>
                            <label className="Input_label__90o4b">{Languages.inputText.codeArea}</label>
                            <select className='form_sellect_control select_phone_code_area' name='form_sellect_stt' value={selectSTT} onChange={onChangeSelectStt}>
                                <option value="1">+84</option>
                            </select>
                        </div>
                    </div>
                    {renderInput('', '', Languages.inputText.phone, Languages.inputText.phonepla, 'number', 30, false, false)}
            ...
            <div className='input_fields_control'>
                <div className='place_title_input'>
                    <label>
                        {Languages.inputText.father}
                    </label>
                </div>
                <div className='group_input_control'>

                    {renderInput('', '', Languages.inputText.firstName, Languages.inputText.firstName, 'text', 15, false, false)}
                    {renderInput('', '', Languages.inputText.firstAnother, Languages.inputText.firstAnother, 'text', 30, false, false)}
                    {renderInput('', '', Languages.inputText.namesingle, Languages.inputText.namesingle, 'text', 30, false, false)}
                    ...
                    {renderInput('', '', Languages.inputText.phone, Languages.inputText.phonepla, 'number', 30, false, false)}
                    {renderInput('', '', Languages.inputText.death, '', 'checkbox', 30, false, false, 'inputStyle')}
                </div>
            </div>
    ...
    const renderFamilyWoman = useMemo(() => {
        ...
        return <div className='total_family_one_side'>
            ...
            <div className='input_fields_control'>
                <div className='place_title_input'>
                    <label>
                        {Languages.inputText.bride}
                    </label>
                </div>
                <div className='group_input_control'>

                    {renderInput('', '', Languages.inputText.firstNameBride, Languages.inputText.firstName, 'text', 15, false, false)}
                    {renderInput('', '', Languages.inputText.firstAnother, Languages.inputText.firstAnother, 'text', 30, false, false)}
                    {renderInput('', '', Languages.inputText.nameGroom, Languages.inputText.namesingle, 'text', 30, false, false)}
                ...
                <div className='group_input_control'>
                    ...
                    <div className='enable_show_deadman deactive'>
                        <div className='label_left'>
                            <label>
                                Chế độ hiển thị với người đã mất:
                            </label>
                        </div>
                        <div className='radio_enable'>
                            ...
                            <RadioButton
                                ...
                                label={<div className='chrysanthemumIcon'>{Languages.text.chrysanthemumIcon} <img src={IcChrysanthemum} /></div>}
            ...
            <div className='my_input_form_data_group'>
                <div className='container mx-auto'>
                    <div className='upload_represent_box'>
                        <div className='md:grid md:grid-cols-3 md:gap-5'>
                            <div className='col-span-2'>
                                {renderImageUploadSingle(Languages.text.chooseCoverImage, imagesCover, Languages.text.bigsize, false, onChangeCoverImage)}
                            </div>
                            {renderImageUploadSingle(Languages.text.chooseThumbs, images, Languages.text.smallsize, false, onChange)}
                        </div>
                    </div>
                    ...
                    <div className='wrapper_information_wedding'>
                        {renderTitle(Languages.text.inforWed, false, 'title_comp_wed_add_cus')}
                        {renderFamilyMan}
                        {renderFamilyWoman}
                    </div>
                </div>
